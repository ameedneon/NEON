...
local files_Neon = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_Neon) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_Neon")
...
local hash = database:sismember('Neon:'..bot_id..'dev', msg.sender_user_id_) 
...
local hash = database:sismember('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_) 
...
local hash = database:sismember('Neon:'..bot_id..'creatorbasic:'..chat,user)
if hash then 
return true 
else 
return false 
...
local hash = database:sismember('Neon:'..bot_id..'creator:'..chat, user) 
if hash then 
return true 
else 
return false 
...
local hash = database:sismember('Neon:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) 
...
local hash = database:sismember('Neon:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)    
...
local hash = database:sismember('Neon:'..bot_id..'owners:'..chat, user) 
if hash then 
return true 
else 
return false 
...
function is_mod(msg)
local hash = database:sismember('Neon:'..bot_id..'mods:'..msg.chat_id_,msg.sender_user_id_)    
...
function is_vip(msg)
local hash = database:sismember('Neon:'..bot_id..'vipgp:'..msg.chat_id_,msg.sender_user_id_) 
...
local hash = database:sismember('Neon:'..bot_id..'SET:BAN'..msg.chat_id_,msg.sender_user_id_) 
if hash then    
return true 
else 
return false 
...
var = true  
elseif database:sismember('Neon:'..bot_id..'dev', user_id) then
var = true  
elseif database:sismember('Neon:'..bot_id..'creatorbasic:'..chat_id, user_id) then
var = true
elseif is_creatorbasicc(chat_id, user_id) then
var = true
elseif database:sismember('Neon:'..bot_id..'creator:'..chat_id, user_id) then
var = true  
elseif database:sismember('Neon:'..bot_id..'owners:'..chat_id, user_id) then
var = true  
elseif database:sismember('Neon:'..bot_id..'mods:'..chat_id, user_id) then
var = true  
elseif database:sismember('Neon:'..bot_id..'vipgp:'..chat_id, user_id) then  
var = true  
else  
var = false  
end  
...
if database:sismember("Neon:gog"..bot_id, chat_id) then
var = true
else 
var = false
end
...
local var = false
local banned = database:sismember('Neon:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('Neon:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('Neon:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('Neon:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
local hash =  'Neon:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
{
{text = ''..(channel_ts or "Neon TEAM")..'', url=''..(channel_user_ts or 't.me/BB8SS')..''},
},
}
local ameed = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
ameed = ameed .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 
end
...
for Neon_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = Neon_one
break
end
end
end 
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'BB8SS')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, ameed333)
end
if value == "prore" then
function get_NeonX(Neonx1,Neonx2,Neonx3)
local id_Neonx = Neonx2.sender_user_id_
function ameed333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'BB8SS')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_Neonx, ameed333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_NeonX)
end
...
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'BB8SS')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
function Neon_run_file(data)
local allfiles = io.popen('ls'):lines()
for Files in allfiles do
if Files == "files_Neon" or Files == "Fastinstall.sh" or Files == "install.sh" or Files == "install_user.sh" or Files == "libs" or Files == "README.md" or Files == "start.lua" or Files == "sudo.lua" or Files == "tg" or Files == "ts" or Files == "TsAu" or Files == "Neon.lua" then
else
...
local files_Neon = database:smembers("files"..bot_id)
for i=1,#files_Neon do
local Neonee = dofile("files_Neon/"..files_Neon[i])
local f = load("files_Neon/"..files_Neon[i].."")
if f ~= "ok" then
if f:match("^(.*)('b')(.*)$") 
or f:match("^(.*)('o')(.*)$") 
or f:match("^(.*)('t')(.*)$") 
...
os.execute("rm -fr files_Neon/*")
return false
end
end
local kt = Neonee.ameed_Neon(data)
if kt == 'end' then
return false
end
end
...
database:set("lock_tag:Neon"..Chat..bot_id,"ok");database:set("lock_sarha:Neon"..Chat..bot_id,"ok");database:set("lock_word:Neon"..Chat..bot_id,"ok");database:set("lock_edit:Neon"..Chat..bot_id,"ok");database:set("lock_lllll:Neon"..Chat..bot_id,"ok");database:set("lock_gif:Neon"..Chat..bot_id,"ok");database:set("lock_files:Neon"..Chat..bot_id,"ok");database:set("lock_mark:Neon"..Chat..bot_id,"ok");database:set("lock_photo:Neon"..Chat..bot_id,"ok");database:set("lock_stecker:Neon"..Chat..bot_id,"ok");database:set("lock_video:Neon"..Chat..bot_id,"ok");database:set("lock_audeo:Neon"..Chat..bot_id,"ok");database:set("lock_voice:Neon"..Chat..bot_id,"ok");database:set("lock_contact:Neon"..Chat..bot_id,"ok");database:set("lock_fwd:Neon"..Chat..bot_id,"ok");database:set("lock_link:Neon"..Chat..bot_id,"ok");database:set("lock_username:Neon"..Chat..bot_id,"ok");database:set("lock_botAndBan:Neon"..Chat..bot_id,"ok");database:set("lock_new:Neon"..Chat..bot_id,"ok")
end
...
database:sadd("Neon:gog"..bot_id,idg) 
database:sadd( 'Neon:'..bot_id.."groups",idg) 
database:set( 'Neon:'..bot_id.."charge:"..idg,true)
Add_local(idg)
if v.MNSH then
for k,idmsh in pairs(v.MNSH) do
database:sadd('Neon:'..bot_id..'creator:'..idg,idmsh)
end;end
if v.MDER then
for k,idmder in pairs(v.MDER) do
database:sadd('Neon:'..bot_id..'owners:'..idg,idmder)  
end;end
if v.MOD then
for k,idmod in pairs(v.MOD) do
database:sadd('Neon:'..bot_id..'mods:'..idg,idmod)  
end;end
if v.ASAS then
for k,idASAS in pairs(v.ASAS) do
database:sadd('Neon:'..bot_id..'creatorbasic:'..idg,idASAS)  
end;end
if v.linkgroup then
if v.linkgroup ~= "" then
database:set('Neon:'..bot_id.."group:link"..idg,v.linkgroup)   
end;end;end
sendtext(chat,msg.id_,"🔰┇تم رفع الملف بنجاح وتفعيل المجموعات\n📬┇ورفع {الامنشئين الاساسين ; والمنشئين ; والمدراء; والادمنيه} بنجاح")   
end
function Neon_run_(msg,data)
local text = msg.content_.text_
------------------------------------------------------------------------
if text == ("مسح ردود المطور") and is_devabas(msg) then
local list = redis:smembers('Neon:'..bot_id..'sudo:ameedre')
for k,v in pairs(list) do
redis:del('Neon:'..bot_id.."sudo:add:reply1:gif"..v)   
redis:del('Neon:'..bot_id.."sudo:add:reply1:vico"..v)   
redis:del('Neon:'..bot_id.."sudo:add:reply1:stekr"..v)     
redis:del('Neon:'..bot_id.."sudo:add:reply:rd"..v)   
redis:del('Neon:'..bot_id.."sudo:addreply1:photo:gp"..v)
redis:del('Neon:'..bot_id.."sudo:addreply1:video:gp"..v)
redis:del('Neon:'..bot_id.."sudo:addreply1:document:gp"..v)
redis:del('Neon:'..bot_id.."sudo:addreply1:audio:gp"..v)
redis:del('Neon:'..bot_id..'sudo:ameedre')
end
...
for k,v in pairs(list) do
if redis:get('Neon:'..bot_id.."sudo:add:reply1:gif"..v) then
db = 'متحركه 🎭'
elseif redis:get('Neon:'..bot_id.."sudo:add:reply1:vico"..v) then
db = 'بصمه 📢'
elseif redis:get('Neon:'..bot_id.."sudo:add:reply1:stekr"..v) then
db = 'ملصق 🃏'
elseif redis:get('Neon:'..bot_id.."sudo:add:reply:rd"..v) then
db = 'رساله ✉'
elseif redis:get('Neon:'..bot_id.."sudo:addreply1:photo:gp"..v) then
db = 'صوره 🎇'
elseif redis:get('Neon:'..bot_id.."sudo:addreply1:video:gp"..v) then
db = 'فيديو 📹'
elseif redis:get('Neon:'..bot_id.."sudo:addreply1:document:gp"..v) then
db = 'ملف 📁'
elseif redis:get('Neon:'..bot_id.."sudo:addreply1:audio:gp"..v) then
db = 'اغنيه 🎵'
end
...
local ameed1 = redis:get('Neon:'..bot_id..'sudo:ameed1'..msg.sender_user_id_..''..msg.chat_id_..'')
local test = redis:get('Neon:'..bot_id..'sudo:msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if ameed1 == 're' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الرد', 1, 'md')
redis:set('Neon:'..bot_id..'sudo:ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
if msg.content_.sticker_ then   
redis:set('Neon:'..bot_id.."sudo:add:reply1:stekr"..test, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set('Neon:'..bot_id.."sudo:add:reply1:vico"..test, msg.content_.voice_.voice_.persistent_id_)  
redis:set('Neon:'..bot_id.."sudo:addreply1:voice:caption:gp"..test,(msg.content_.caption_ or ''))  
end   
if msg.content_.animation_ then   
redis:set('Neon:'..bot_id.."sudo:add:reply1:gif"..test, msg.content_.animation_.animation_.persistent_id_)  
redis:set('Neon:'..bot_id.."sudo:addreply1:animation:caption:gp"..test,(msg.content_.caption_ or ''))  
end  
if text then   
text = text:gsub('"','') 
text = text:gsub("'",'') 
text = text:gsub('`','') 
text = text:gsub('*','') 
redis:set('Neon:'..bot_id.."sudo:add:reply:rd"..test, text)  
end  
if msg.content_.audio_ then
redis:set('Neon:'..bot_id.."sudo:addreply1:audio:gp"..test, msg.content_.audio_.audio_.persistent_id_)  
redis:set('Neon:'..bot_id.."sudo:addreply1:audio:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('Neon:'..bot_id.."sudo:addreply1:document:gp"..test, msg.content_.document_.document_.persistent_id_)  
redis:set('Neon:'..bot_id.."sudo:addreply1:document:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('Neon:'..bot_id.."sudo:addreply1:video:gp"..test, msg.content_.video_.video_.persistent_id_)  
redis:set('Neon:'..bot_id.."sudo:addreply1:video:caption:gp"..test,(msg.content_.caption_ or ''))  
end
...
redis:set('Neon:'..bot_id.."sudo:addreply1:photo:gp"..test, photo_in_group)  
redis:set('Neon:'..bot_id.."sudo:addreply1:photo:caption:gp"..test,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
redis:set('Neon:'..bot_id..'sudo:ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('Neon:'..bot_id..'sudo:msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:del('Neon:'..bot_id.."sudo:add:reply1:gif"..text)   
redis:del('Neon:'..bot_id.."sudo:add:reply1:vico"..text)   
redis:del('Neon:'..bot_id.."sudo:add:reply1:stekr"..text)     
redis:del('Neon:'..bot_id.."sudo:add:reply:rd"..text)   
redis:del('Neon:'..bot_id.."sudo:addreply1:photo:gp"..text)
redis:del('Neon:'..bot_id.."sudo:addreply1:video:gp"..text)
redis:del('Neon:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('Neon:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:sadd('Neon:'..bot_id..'sudo:ameedre', text)
return false end
end
if text and text:match("^(.*)$") then
local ameed1 = redis:get('Neon:'..bot_id..'sudo:ameed1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('Neon:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('Neon:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:del('Neon:'..bot_id..'sudo:ameed1'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:srem('Neon:'..bot_id..'sudo:ameedre', text)
return false
end
end
if text == 'اضف رد للكل' and is_devabas(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('Neon:'..bot_id..'sudo:ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false 
end
if text == 'حذف رد للكل' and is_devabas(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ', 1, 'md')
redis:set('Neon:'..bot_id..'sudo:ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false 
end
if text and not database:get('Neon:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local anemi = redis:get('Neon:'..bot_id.."sudo:add:reply1:gif"..text)   
local veico = redis:get('Neon:'..bot_id.."sudo:add:reply1:vico"..text)   
local stekr = redis:get('Neon:'..bot_id.."sudo:add:reply1:stekr"..text)     
local text1 = redis:get('Neon:'..bot_id.."sudo:add:reply:rd"..text)   
local photo = redis:get('Neon:'..bot_id.."sudo:addreply1:photo:gp"..text)
local video = redis:get('Neon:'..bot_id.."sudo:addreply1:video:gp"..text)
local document = redis:get('Neon:'..bot_id.."sudo:addreply1:document:gp"..text)
local audio = redis:get('Neon:'..bot_id.."sudo:addreply1:audio:gp"..text)
------------------------------------------------------------------------
local video_caption = (redis:get('Neon:'..bot_id.."sudo:addreply1:video:caption:gp"..text) or '' )
local photo_caption = (redis:get('Neon:'..bot_id.."sudo:addreply1:photo:caption:gp"..text) or '' )
local document_caption = (redis:get('Neon:'..bot_id.."sudo:addreply1:document:caption:gp"..text) or '' )
local audio_caption = (redis:get('Neon:'..bot_id.."sudo:addreply1:audio:caption:gp"..text) or '' )
local animation_caption = (redis:get('Neon:'..bot_id.."sudo:addreply1:animation:caption:gp"..text) or '' )
local voice_caption = (redis:get('Neon:'..bot_id.."sudo:addreply1:voice:caption:gp"..text) or '' )
------------------------------------------------------------------------
if text1 then 
function Neon_re(t1,t2)
local tahna = (database:get('Neon:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = database:get('Neon:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي'
elseif is_sudo(msg) then
t = database:get("Neon:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = database:get("Neon:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Neon:all_sl:"..database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("Neon:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Neon:all_sl:"..database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("Neon:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = database:get("Neon:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = database:get("Neon:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = database:get("Neon:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
getUser(msg.sender_user_id_, Neon_re)
end
if stekr then 
...
local list = redis:smembers('Neon:'..bot_id..'ameedre'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('Neon:'..bot_id.."add:reply1:gif"..v..msg.chat_id_)   
redis:del('Neon:'..bot_id.."add:reply1:vico"..v..msg.chat_id_)   
redis:del('Neon:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('Neon:'..bot_id.."add:reply:rd"..v..msg.chat_id_)   
redis:del('Neon:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_)
redis:del('Neon:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_)
redis:del('Neon:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_)
redis:del('Neon:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_)
redis:del('Neon:'..bot_id..'ameedre'..msg.chat_id_..'')
end
...
for k,v in pairs(list) do
if redis:get('Neon:'..bot_id.."add:reply1:gif"..v..msg.chat_id_) then
db = 'متحركه 🎭'
elseif redis:get('Neon:'..bot_id.."add:reply1:vico"..v..msg.chat_id_) then
db = 'بصمه 📢'
elseif redis:get('Neon:'..bot_id.."add:reply1:stekr"..v..msg.chat_id_) then
db = 'ملصق 🃏'
elseif redis:get('Neon:'..bot_id.."add:reply:rd"..v..msg.chat_id_) then
db = 'رساله ✉'
elseif redis:get('Neon:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_) then
db = 'صوره 🎇'
elseif redis:get('Neon:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_) then
db = 'فيديو 📹'
elseif redis:get('Neon:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_) then
db = 'ملف 📁'
elseif redis:get('Neon:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_) then
db = 'اغنيه 🎵'
end
text = text.."<b>|"..k.."|</b>~⪼("..v..") » {"..db.."}\n"
if #text > 7000 then
...
if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or msg.content_.document_ or msg.content_.photo_ or msg.content_.video_ then  
local ameed1 = redis:get('Neon:'..bot_id..'ameed1'..msg.sender_user_id_..''..msg.chat_id_..'')
local test = redis:get('Neon:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if ameed1 == 're' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الرد', 1, 'md')
redis:set('Neon:'..bot_id..'ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
if msg.content_.sticker_ then   
redis:set('Neon:'..bot_id.."add:reply1:stekr"..test..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set('Neon:'..bot_id.."add:reply1:vico"..test..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_)  
redis:set('Neon:'..bot_id.."addreply1:voice:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end   
if msg.content_.animation_ then   
redis:set('Neon:'..bot_id.."add:reply1:gif"..test..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_)  
redis:set('Neon:'..bot_id.."addreply1:animation:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end  
if text then   
text = text:gsub('"','') 
text = text:gsub("'",'') 
text = text:gsub('`','') 
text = text:gsub('*','') 
redis:set('Neon:'..bot_id.."add:reply:rd"..test..msg.chat_id_, text)  
end  
if msg.content_.audio_ then
redis:set('Neon:'..bot_id.."addreply1:audio:gp"..test..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_)  
redis:set('Neon:'..bot_id.."addreply1:audio:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('Neon:'..bot_id.."addreply1:document:gp"..test..msg.chat_id_, msg.content_.document_.document_.persistent_id_)  
redis:set('Neon:'..bot_id.."addreply1:document:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('Neon:'..bot_id.."addreply1:video:gp"..test..msg.chat_id_, msg.content_.video_.video_.persistent_id_)  
redis:set('Neon:'..bot_id.."addreply1:video:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
redis:set('Neon:'..bot_id.."addreply1:photo:gp"..test..msg.chat_id_, photo_in_group)  
redis:set('Neon:'..bot_id.."addreply1:photo:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
if ameed1 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('Neon:'..bot_id..'ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('Neon:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:del('Neon:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
redis:del('Neon:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
redis:del('Neon:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('Neon:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
redis:del('Neon:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
redis:del('Neon:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
redis:del('Neon:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('Neon:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:sadd('Neon:'..bot_id..'ameedre'..msg.chat_id_..'', text)
return false end
end
if text and text:match("^(.*)$") then
local ameed1 = redis:get('Neon:'..bot_id..'ameed1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('Neon:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('Neon:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:del('Neon:'..bot_id..'ameed1'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:srem('Neon:'..bot_id..'ameedre'..msg.chat_id_..'', text)
return false
end
end
if text == 'اضف رد' and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('Neon:'..bot_id..'ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false 
end
if text == 'حذف رد' and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ', 1, 'md')
redis:set('Neon:'..bot_id..'ameed1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false 
end
if text and not database:get('Neon:'..bot_id..'repowner:mute'..msg.chat_id_) then
local anemi = redis:get('Neon:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
local veico = redis:get('Neon:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
local stekr = redis:get('Neon:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
local text1 = redis:get('Neon:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
local photo = redis:get('Neon:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
local video = redis:get('Neon:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
local document = redis:get('Neon:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
local audio = redis:get('Neon:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
------------------------------------------------------------------------
local video_caption = (redis:get('Neon:'..bot_id.."addreply1:video:caption:gp"..text..msg.chat_id_) or '' )
local photo_caption = (redis:get('Neon:'..bot_id.."addreply1:photo:caption:gp"..text..msg.chat_id_) or '' )
local document_caption = (redis:get('Neon:'..bot_id.."addreply1:document:caption:gp"..text..msg.chat_id_) or '' )
local audio_caption = (redis:get('Neon:'..bot_id.."addreply1:audio:caption:gp"..text..msg.chat_id_) or '' )
local animation_caption = (redis:get('Neon:'..bot_id.."addreply1:animation:caption:gp"..text..msg.chat_id_) or '' )
local voice_caption = (redis:get('Neon:'..bot_id.."addreply1:voice:caption:gp"..text..msg.chat_id_) or '' )
------------------------------------------------------------------------
if text1 then 
function Neon_re(t1,t2)
local tahna = (database:get('Neon:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
...
------------------------------------------------------------------------
if database:get('Neon:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and is_sudo(msg) then 
if text and text:match("^الغاء$") or text and text:match("^الغاء ✖$") then   
send(msg.chat_id_, msg.id_, 1, "*📬┇ تم الغاء الاذاعه للمشتركين *\n", 1, "md") 
database:del('Neon:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
else 
local gps = database:scard('Neon:'..bot_id.."userss") or 0 
if msg.content_.text_ then
whatbc = 'الرساله'
local list = database:smembers('Neon:'..bot_id..'userss') 
for k,v in pairs(list) do 
...
------------------------------------------------------------------------
if database:get("Neon:set_if_bc_new:"..bot_id..msg.sender_user_id_) and is_sudo(msg) then  
database:del("Neon:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('Neon:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
for i=1, #gpss do
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
for i=1, #gpss do
...
------------------------------------------------------------------------
if database:get("Neon:set_if_bc_new:pin"..bot_id..msg.sender_user_id_) and is_sudo(msg) then  
database:del("Neon:set_if_bc_new:pin"..bot_id..msg.sender_user_id_)
local pro = database:scard('Neon:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
for i=1, #gpss do
send(gpss[i], 0, 1, text, 1, 'html')	
database:set('Neon:'..bot_id..'Msg_Text'..gpss[i],text) 
end					
end
if (data.message_.content_.sticker_) then 
...
for i=1, #gpss do
tdcli.sendSticker(gpss[i], 0,0, 1, nil, data.message_.content_.sticker_.sticker_.persistent_id_)
database:set('Neon:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.sticker_.sticker_.persistent_id_) 
end
elseif (data.message_.content_.voice_) then 
gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendVoice(gpss[i], 0,0, 1, nil, data.message_.content_.voice_.voice_.persistent_id_)
database:set('Neon:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.voice_.voice_.persistent_id_) 
end
elseif (data.message_.content_.video_) then 
gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendVideo(gpss[i], 0,0, 1, nil, data.message_.content_.video_.video_.persistent_id_)
database:set('Neon:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.video_.video_.persistent_id_) 
end
elseif (data.message_.content_.animation_) then 
gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendDocument(gpss[i], 0,0, 1, nil, data.message_.content_.animation_.animation_.persistent_id_)
database:set('Neon:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.animation_.animation_.persistent_id_) 
end
elseif (data.message_.content_.document_) then
gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendDocument(gpss[i], 0,0, 1, nil, data.message_.content_.document_.document_.persistent_id_)
database:set('Neon:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.document_.document_.persistent_id_) 
end
elseif (data.message_.content_.photo_) then
local id_ameed = nil
if data.message_.content_.photo_.sizes_[0] then
...
for i=1, #gpss do
tdcli.sendPhoto(gpss[i], 0, 0, 1, nil, id_ameed,(msg.content_.caption_ or " "))		
database:set('Neon:'..bot_id..'Msg_Text'..gpss[i],id_ameed) 
end
end
gpss = database:smembers( 'Neon:'..bot_id.."groups") or 0
send(msg.chat_id_, msg.id_, 1, '☑┇تم نشر الرساله في {'..(#gpss)..'} مجموعه ', 1, 'md')
...
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local ameed2 = redis:get('Neon:'..bot_id..'texts'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Neon:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local ameed2 = redis:get('Neon:'..bot_id..'textst'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ تم حفظ الاسم ', 1, 'html')
redis:set('Neon:'..bot_id..'textst'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'name_bot', text)
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^+(.*)$") then
local ameedo = redis:get('Neon:'..bot_id..'sudoo'..text..'')
local ameed2 = redis:get('Neon:'..bot_id..'nameed'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('Neon:'..bot_id..'nmameed', text)
redis:set('Neon:'..bot_id..'nameed'..msg.sender_user_id_..'', 'mmsg')
return false 
end
end
------------------------------------------------------------------------
...
redis:set('Neon:'..bot_id..'nameed'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'naameed', text)
local nmameed = redis:get('Neon:'..bot_id..'nmameed')
...
database:set("Neon:gr:id:text:"..bot_id..msg.chat_id_,text)
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local ameed2 = redis:get('Neon:'..bot_id..'h44'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Neon:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local ameed2 = redis:get('Neon:'..bot_id..'h33'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Neon:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local ameed2 = redis:get('Neon:'..bot_id..'h22'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Neon:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local ameed2 = redis:get('Neon:'..bot_id..'h11'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Neon:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local ameed2 = redis:get('Neon:'..bot_id..'hhh'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Neon:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
local ameed2 = redis:get('Neon:'..bot_id..'hres1'..msg.sender_user_id_..'')
if ameed2 == 'msg' then
if text:match("^(.*)$") then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Neon:'..bot_id..'hres1'..msg.sender_user_id_..'', 'no')
redis:set('Neon:'..bot_id..'hres', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
...
if database:get("lock_kansers:Neon"..msg.chat_id_..bot_id) then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
local last_ = data.last_name_ or ''
local first_ = data.first_name_ or ''
taha = (first_..''..last_)
Num = (database:get('Neon:'..bot_id..'Num:kansers'..msg.chat_id_) or 25)
...
if database:get('Neon:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('Neon:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
local taha = database:get('Neon:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)   
if taha then    
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┇["..taha.."] \n" , 1, 'md') 
else
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/BB8SS)}\n📛┇["..taha.."] \n" , 1, 'md') 
end
end
...
local filter = database:smembers('Neon:'..bot_id.."filtersteckr"..msg.chat_id_)
...
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/BB8SS)}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_,{[0] = msg.id_})       
...
local filter = database:smembers('Neon:'..bot_id.."filterphoto"..msg.chat_id_)
for k,v in pairs(filter) do
...
local filter = database:smembers('Neon:'..bot_id.."filteranimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if database:get('Neon:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:Neon"..msg.chat_id_..bot_id) then
local pin_id = database:get('Neon:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
if database:get("lock_link.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
...
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
end
------------------------------------------------------------------------
...
if text and text:match("(.*)(@)(.*)")  then
if database:get("lock_username.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if database:get("lock_chat.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_fwd.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_stecker.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_note.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_photo.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_audeo.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_voice.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_video.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_gif.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_contect.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_ar.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_files.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_en.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_mark.note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
...
------------------------------------------------------------------------
if text and text:match("(.*)") then
if database:get("lock_word:Neon"..msg.chat_id_..bot_id) then 
local Neon_wr = (database:get("Neon:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(Neon_wr) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_link:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
local text = msg.content_.caption_
if text and text:match("(.*)(@)(.*)")  then
if database:get("lock_username:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
------------------------------------------------------------------------
if database:get("lock_chat:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
------------------------------------------------------------------------
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_reple:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if  msg.content_.ID == "MessageChatJoinByLink" then
if database:get("lock_join:Neon"..msg.chat_id_..bot_id) then
...
if database:get("lock_bot:Neon"..msg.chat_id_..bot_id) and not bot_id then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:Neon"..msg.chat_id_..bot_id) then
...
if msg.forward_info_ then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
if database:get("lock_fwd:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_stecker:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_new:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if msg.content_.entities_[0] then
if msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
if database:get("lock_mark:Neon"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('Neon:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
if not is_vip(msg) then
if database:get("lock_join:Neon"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
return false
...
------------------------------------------------------------------------
if database:get("lock_lllll:Neon"..msg.chat_id_..bot_id)  and msg.content_.ID ~= "MessageChatAddMembers" then  
...
local hash = 'Neon:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:set("lock_media:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_word:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_mark:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_link:Neon"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:Neon"..msg.chat_id_..bot_id,"ok")
database:set('Neon:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:sadd('Neon:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
------------------------------------------------------------------------
if msg.content_.photo_ then
local photo = database:get('Neon:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
if msg.content_.photo_.sizes_[0] then
...
database:del('Neon:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false
end 
end
------------------------------------------------------------------------
...
if not database:sismember( 'Neon:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'Neon:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('Neon:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'Neon:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'Neon:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
------------------------------------------------------------------------
if database:get('Neon:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
database:del('Neon:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
local url , res = http.request('http://Neon.ml/joinch/?id='..msg.sender_user_id_..'')
data = JSON.decode(url)
if data.Ch_Member.Neon ~= true then
...
if not database:get( 'Neon:'..bot_id.."charge:"..msg.chat_id_) then
...
if not database:get( 'Neon:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'Neon:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."✔️┇تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
database:sadd("Neon:gog"..bot_id, msg.chat_id_)
function thsake_info2(k1,k2)
function dl_cb222(t1,t2)
...
database:set('Neon:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("Neon"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('Neon:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'Neon:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('Neon:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
database:sadd('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
local list = database:smembers('Neon:'..bot_id.."groups")  
local t = '{"BOT_ID": '..bot_id..',"GP_BOT":{'  
for k,v in pairs(list) do   
NAME = 'Neon Chat'
link = database:get('Neon:'..bot_id.."group:link"..v) or ''
MNSH = database:smembers('Neon:'..bot_id..'creator:'..v)
MDER = database:smembers('Neon:'..bot_id..'owners:'..v)
MOD = database:smembers('Neon:'..bot_id..'mods:'..v)
ASAS = database:smembers('Neon:'..bot_id..'creatorbasic:'..v)
if k == 1 then
t = t..'"'..v..'":{"Neon":"'..NAME..'",'
else
t = t..',"'..v..'":{"Neon":"'..NAME..'",'
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/Neonabas/files_Neon/master/getfile.json")
if res == 200 then
...
local Check_File_is_Found = io.open("files_Neon/"..name,"r")
...
local file_bot = io.open("files_Neon/"..file,"r")
if file_bot then
io.close(file_bot)
...
local json_file, res = https.request("https://raw.githubusercontent.com/Neonabas/files_Neon/master/files_Neon/"..file)
if res == 200 then
os.execute("rm -fr files_Neon/"..file)
...
local json_file, res = https.request("https://raw.githubusercontent.com/Neonabas/files_Neon/master/files_Neon/"..file)
if res == 200 then
local chek = io.open("files_Neon/"..file,'w+')
...
local Neone = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Neone .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_Neon/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end

if (text == 'الملفات' ) then
local files_Neon = database:smembers("files"..bot_id)
local files = io.popen('cd files_Neon && ls'):read("*all")
local files_Neon2 = ''
for i=1,#files_Neon do
files_Neon2 = files_Neon2..'{'..files_Neon[i]..'}\n'
end
...
database:del('Neon:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
if text == ("مسح المكتومين عام") then
text = '☑┇ تم مسح المكتومين عام'
database:del('Neon:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
------------------------------------------------------------------------
if text == ("قائمه العام") then
local hash =   'Neon:'..bot_id..'gbanned:'
local list = database:smembers(hash)
text = "⛔┇قائمة الحظر العام  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = database:hgetall('Neon:'..bot_id..'user:'..v)
...
local hash =  'Neon:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'BB8SS')..')\n🚫┇تم حظره من المجموعات البوت'
...
local hash =  'Neon:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'BB8SS')..')\n��┇تم الغاء حظره من المجموعات البوت'
...
if redis:sismember('Neon:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'🔖┇بالفعل تم رفعة مطور في البوت')
else
redis:set('Neon:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('Neon:'..bot_id..'dev', result.sender_user_id_)
...
redis:set('Neon:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('Neon:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'BB8SS')..')\n\n🔖┇تم رفعة مطور في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
redis:set('Neon:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('Neon:'..bot_id..'dev', apmd[2])
...
if not redis:sismember('Neon:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'بالفعل تم تنزيلة من مطورين البوت')
else
redis:del('Neon:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('Neon:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:del('Neon:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('Neon:'..bot_id..'dev', result.id_)
...
redis:del('Neon:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('Neon:'..bot_id..'dev', apmd[2])
...
os.execute('cd libs;wget https://raw.githubusercontent.com/ameedNeon/Neon/master/libs/utils.lua')
...
local gps = database:scard('Neon:'..bot_id.."groups")  
...
redis:del('Neon:'..bot_id..'name_bot')
...
local gps = database:scard('Neon:'..bot_id.."userss")  
...
local pv = database:smembers('Neon:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
...
database:srem('Neon:'..bot_id.."userss",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = database:smembers('Neon:'..bot_id..'groups') 
local w = 0
local q = 0
for i = 1, #group do
...
database:srem("Neon:gog"..bot_id,group[i]) 
database:srem('Neon:'..bot_id..'pro:groups',group[i]) 
database:srem( 'Neon:'..bot_id.."groups",group[i]) 
...
local gps = database:smembers('Neon:'..bot_id.."groups") or 0
local gps2 = database:smembers("Neon:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("Neon:gog"..bot_id, gps[i])
database:set('Neon:'..bot_id.."enable:"..gps[i],true)
database:set( 'Neon:'..bot_id.."charge:"..gps[i],true)
end
...
local gps2 = database:smembers("Neon:gog"..bot_id) or 0
for i=1,#gps do
database:del("Neon:gog"..bot_id)
database:del('Neon:'..bot_id.."enable:"..gps[i])
database:del( 'Neon:'..bot_id.."charge:"..gps[i])
end
...
database:del("Neon:gog"..bot_id)
database:del('Neon:'..bot_id.."groups")
end
------------------------------------------------------------------------
...
if database:get('Neon:'..bot_id.."group:link"..gpss[i]) then 
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(database:get('Neon:'..bot_id.."group:link"..gpss[i])).."\n"
else
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link.result or ' لا توجد روابط ').."\n"
end
end
local f = io.open('Neon_Links.txt', 'w')
f:write(text233)
f:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './Neon_Links.txt', '', nil)  
end
------------------------------------------------------------------------
...
for i=1, #gpss do
local link = database:get('Neon:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('Neon.txt', 'w')
f:write(text233)
f:close()
local Neone = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Neone .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'Neon.txt' .. '"'
io.popen(curl)
end
...
redis:set('Neon:'..bot_id..'nameed'..msg.sender_user_id_..'', 'msg')
return false 
end
if text == ("مسح المطورين") then
local list = redis:smembers('Neon:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('Neon:'..bot_id..'dev')
redis:del('Neon:'..bot_id..'sudoo'..v)
end
...
if database:sismember('Neon:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('Neon:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') then
if not database:sismember('Neon:'..bot_id..'pro:groups',msg.chat_id_) then
...
redis:set('Neon:'..bot_id..'hres1'..msg.sender_user_id_..'', 'msg')
return false 
end

if (text and text == 'تغير امر م2') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Neon:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م3') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Neon:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م4') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Neon:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('Neon:'..bot_id..'text_sudo', text)
...
if (not database:get('Neon:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("Neon:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "Neon"
else
...
if (not database:get('Neon:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("Neon:set_if_bc_new:pin"..bot_id..msg.sender_user_id_,"ok")
return "Neon"
else
send(msg.chat_id_, msg.id_, 1, '☑┇ الاذاعه معطله ', 1, 'md')
end
...
if (not database:get('Neon:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
database:setex('Neon:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
...
database:set('Neon:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("Neon"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n📌┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
------------------------------------------------------------------------
if text == 'مغادره' then
local lave = database:get('Neon:'..bot_id..'leave:groups')
print(lave)
if not database:get('Neon:'..bot_id..'leave:groups') then
...
database:del('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
...
return false end
function promote_by_reply(extra, result, success)
database:sadd('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
database:sadd('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:sadd('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
return false end
function promote_by_reply(extra, result, success)
database:srem('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:srem('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"تم تنزيلة منشئ اساسي من البوت")
return false
end
------------------------------------------------------------------------
if (text == ("عدد الكروبات") or text ==("الاحصائيات")) then
local gps = database:scard('Neon:'..bot_id.."groups") or 0
local user = database:scard('Neon:'..bot_id.."userss") or 0
local gps2 = database:scard("Neon:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('Neon:'..bot_id..'pro:groups') or 0
...
local time = a[2] * day
database:setex( 'Neon:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'Neon:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'Neon:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:del("Neon:gog"..bot_id,txt[2])
...
database:setex( 'Neon:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'Neon:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, ameed333)
end
------------------------------------------------------------------------
...
database:setex( 'Neon:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function ameed333(extra,result,success)
ameed_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'Neon:'..bot_id.."charge:"..txt[2],true)
...
local list = database:smembers('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_)
if #list == 0 then
...
local hash =  'Neon:'..bot_id..'creator:'..msg.chat_id_
...
database:sadd('Neon:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'BB8SS')..')\n🔖┇تم رفعة منشئ  في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @zx_xx ⚜️\n', 1, 'html')   
return false end
database:sadd('Neon:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
database:del('Neon:'..bot_id..'creator:'..msg.chat_id_)
...
local tsX_o = database:get("lock_edit:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل ")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل ")
database:set("lock_edit:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل بالطرد") then
local tsX_o = database:get("lock_edit:Neon:kick"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل بالطرد")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل بالطرد")
database:set("lock_edit:Neon:kick"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:Neon:kick"..msg.chat_id_..bot_id,"ok")
end
end
if text == 'مسح المدراء' then
database:del('Neon:'..bot_id..'owners:'..msg.chat_id_)
...
local hash =  'Neon:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
database:sadd('Neon:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('Neon:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
database:set("Neon:mute:deleta:msg:"..bot_id..msg.chat_id_,"NeonE")
send(msg.chat_id_, msg.id_, 1, "🔌┇تم تعطيل خاصيه الابلاغ", 1, 'html')
end
if (text and text == "تفعيل الابلاغ") then 
database:del("Neon:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
database:del('Neon:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = database:get('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
database:set('Neon:'..bot_id.."Neon:lock:set"..msg.chat_id_,"Neon")
...
database:del('Neon:'..bot_id.."Neon:lock:set"..msg.chat_id_)
...
database:set("Neon:lock:ban_and_kick"..bot_id..msg.chat_id_,"Neon")
send(msg.chat_id_, msg.id_, 1, '🔌┇تم تعطيل (طرد - حضر) الاعضاء', 1, 'md')
end
if (text and (text == "تفعيل الطرد" or text == "تفعيل الحظر")) then
database:del("Neon:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
database:set("Neon:name_sudo"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------------------
if text and text:match("^تغير رد المنشئ (.*)$") then
ac = {string.match(text, "^تغير رد المنشئ (.*)$")}                     
database:set("Neon:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
...
database:del("Neon:all_sl:"..text..bot_id..msg.chat_id_)
database:srem('Neon:'..bot_id..'Neonnwe:slahea:'..msg.chat_id_,text)
database:del("tsahke:is_del_sl:"..bot_id..msg.chat_id_..msg.sender_user_id_)
end 
if (text and text == "اضف صلاحيه") then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("Neon:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Neon"
end
if text and database:get("Neon:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("Neon:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("Neon:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("Neon:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
database:sadd('Neon:'..bot_id..'Neonnwe:slahea'..msg.chat_id_,text)
return "Neon"
end
if (text and database:get("Neon:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("Neon:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("Neon:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("Neon:all_sl:"..tsha_text..bot_id..msg.chat_id_,text)
database:set("Neonnwe:slahea:bot"..tsha_text..bot_id..msg.chat_id_,text)
database:del("Neon:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
if text =='مسح الصلاحيات' then 
local list = database:smembers('Neon:'..bot_id..'Neonnwe:slahea'..msg.chat_id_)
for k,v in pairs(list) do
local list = database:smembers('Neon:'..bot_id..'Neon:users:sl'..msg.chat_id_)
for k,ks in pairs(list) do
ts_text = database:get("Neon:all_sl:"..v..bot_id..msg.chat_id_)
if ts_text then 
database:del("Neon:name_user:"..bot_id..msg.chat_id_..ks)
end
end
database:del("Neon:all_sl:"..v..bot_id..msg.chat_id_)
database:del("Neonnwe:slahea:bot"..v..bot_id..msg.chat_id_)
end
database:del('Neon:'..bot_id..'Neonnwe:slahea'..msg.chat_id_)
...
for k,v in pairs(list) do
var = database:get("Neonnwe:slahea:bot"..v..bot_id..msg.chat_id_)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
...
database:set('Neon:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
database:sadd('Neon:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
...
database:sadd('Neon:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
database:srem('Neon:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,by_reply)
end
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('Neon:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
local Neon_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
database:set("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id,Neon_edis[1])
database:sadd('Neon:'..bot_id..'Neonnwe:'..msg.chat_id_,Neon_edis[1])
...
database:set("Neon:edit:text:su:newamr:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("Neon:edit:text:su:newamr2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
local klma = database:get("Neon:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
if klma ~= nil then
database:del("Neon:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("Neon:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
database:srem('Neon:'..bot_id..'Neonnwe:'..msg.chat_id_,klma)
...
local list = database:smembers('Neon:'..bot_id..'Neonnwe:'..msg.chat_id_)
if #list == 0 then
...
for k,v in pairs(list) do
var = database:get("Neon:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
...
local list = database:smembers('Neon:'..bot_id..'Neonnwe:'..msg.chat_id_)
for k,v in pairs(list) do
local klma = database:get("Neon:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if klma ~= nil then
database:del("Neon:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("Neon:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
end
end
database:del('Neon:'..bot_id..'Neonnwe:'..msg.chat_id_)
...
database:set('Neon:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @zx_xx ⚜️\n', 1, 'html')   
return false end
if not is_creator(msg) and database:get('Neon:'..bot_id.."Neon:lock:set"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع الرفع تم تعطيل الرفع من قبل المنشئين \n', 1, 'md')
return false
end
function promote_by_reply(extra, result, success)
local hash =  'Neon:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
database:sadd('Neon:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @zx_xx ⚜️\n', 1, 'html')   
return false end
database:sadd('Neon:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
database:del('Neon:'..bot_id..'mods:'..msg.chat_id_)
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'Neon')
return "Neon"
end

if text and text == "مسح الايدي" then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("Neon:gr:id:text:"..bot_id..msg.chat_id_)
end
...
database:set('Neon:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set('Neon:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
...
i = i + 1
local user_info = database:hgetall('Neon:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
if not database:get('Neon:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني بالفعل تم تفعيلة ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني تم تفعيلة ', 1, 'md')
database:del('Neon:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') then
if database:get('Neon:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني بالفعل ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني ', 1, 'md')
database:set('Neon:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end
if text and text:match("^رفع (.*)$")  and msg.reply_to_message_id_ then
a = {string.match(text, "^رفع (.*)$")}
ts_text = database:get("Neon:all_sl:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('Neon:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('Neon:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("Neon:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
database:sadd('Neon:'..bot_id..'Neon:users:sl'..msg.chat_id_,result.sender_user_id_)
function ameed33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'BB8SS')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
ts_text = database:get("Neon:all_sl:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
...
database:sadd('Neon:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:sadd('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:sadd('Neon:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
database:set("Neon:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
database:sadd('Neon:'..bot_id..'Neon:users:sl'..msg.chat_id_,result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'BB8SS')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('Neon:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:sadd('Neon:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('Neon:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("Neon:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
database:sadd('Neon:'..bot_id..'Neon:users:sl'..msg.chat_id_,apmd[2])
...
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:srem('Neon:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:srem('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:srem('Neon:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:del("Neon:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
database:srem('Neon:'..bot_id..'Neon:users:sl'..msg.chat_id_,result.sender_user_id_)
...
if ts_text == "ادمن" then 
database:srem('Neon:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('Neon:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'BB8SS')..')\n🔰┇تم تنزيله من '..apmd[1]
database:srem('Neon:'..bot_id..'Neon:users:sl'..msg.chat_id_,result.id_)
database:del("Neon:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('Neon:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
database:sadd('Neon:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
database:sadd('Neon:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
if b.content_.sticker_ then
local idsticker = b.content_.sticker_.set_id_
database:srem('Neon:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
database:srem('Neon:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.animation_.animation_ then
local idanimation = b.content_.animation_.animation_.persistent_id_
database:srem('Neon:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
...
database:set('Neon:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
...
database:del('Neon:'..bot_id..'lock_geam'..msg.chat_id_) 
end
...
local tsX_o = database:get("lock_tag:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التاك ")
else
tsX000("lock",msg,"🔐┇ تم قفل التاك ")
database:set("lock_tag:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الرد") then
local tsX_o = database:get("lock_reple:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الرد على الرسائل ")
else
tsX000("lock",msg,"🔐┇ تم قفل الرد على الرسائل ")
database:set("lock_reple:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الشارحه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الشارحه ")
database:set("lock_sarha:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_lllll:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التكرار ")
else
tsX000("lock",msg,"🔐┇ تم قفل التكرار ")
database:set("lock_lllll:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_files:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملفات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملفات ")
database:set("lock_files:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_photo:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_stecker:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_video:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_inline:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانلاين ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانلاين ")
database:set("lock_inline:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدردشه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدردشه ")
database:set("lock_chat:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التوجيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل التوجيه ")
database:set("lock_fwd:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التثبيت ")
else
tsX000("lock",msg,"🔐┇ تم قفل التثبيت ")
database:set("lock_pin:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_voice:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_contact:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الجهات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الجهات ")
database:set("lock_contact:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل العربيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل العربيه ")
database:set("lock_ar:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانكليزيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانكليزيه ")
database:set("lock_en:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_link:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_username:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المعرف ")
else
tsX000("lock",msg,"🔐┇ تم قفل المعرف ")
database:set("lock_username:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_botAndBan:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات بالطرد ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات بالطرد ")
database:set("lock_botAndBan:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات ")
database:set("lock_bot:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = database:get("lock_join:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدخول للمجموعه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدخول للمجموعه ")
database:set("lock_join:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_tag:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_sarha:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_word:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_lllll:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_files:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_mark:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_contact:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_fwd:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_link:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_username:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_botAndBan:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_new:Neon"..msg.chat_id_..bot_id,"ok")
...
database:del("lock_reple:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:Neon"..msg.chat_id_..bot_id)
end
end
...
database:del("lock_inline:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:Neon"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_join:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:Neon"..msg.chat_id_..bot_id,"ok")
end
end
if text and text:match('^وضع عدد الكانسر (%d+)$') then
local Num = text:match('^وضع عدد الكانسر (%d+)$')
database:set('Neon:'..bot_id..'Num:kansers'..msg.chat_id_,Num)
send(msg.chat_id_, msg.id_, 1, '\n🔖┇تم وضع عدد حروف الاسم {'..Num..'} حرف',1, 'md')
end
if (text == "قفل الكانسر") then
local tsX_o = database:get("lock_kansers:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الكانسر ")
else
tsX000("lock",msg,"🔐┇ تم قفل الكانسر ")
database:set("lock_kansers:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_kansers:Neon"..msg.chat_id_..bot_id,"ok")
end
end
...
return false end
function promote_by_reply(extra, result, success)
local hash =  'Neon:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if not is_creator(msg) and not is_setban(msg) and database:get("Neon:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'Neon:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('Neon:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'BB8SS')..')\n☑┇تم حظره من المجموعه'
...
database:sadd('Neon:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'Neon:'..bot_id..'banned:'..msg.chat_id_
local hash1 =  'Neon:'..bot_id..'banned:'..msg.chat_id_
...
database:del('Neon:'..bot_id..'banned:'..msg.chat_id_, result.id_)
database:del('Neon:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'BB8SS')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
database:del('Neon:'..bot_id..'banned:'..msg.chat_id_, apba[2])
database:del('Neon:'..bot_id..'muted:'..msg.chat_id_, apba[2])
...
database:set('Neon:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
database:srem('Neon:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('Neon:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
function mute_by_reply(extra, result, success)
local hash =  'Neon:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) == true then
...
database:sadd('Neon:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'BB8SS')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('Neon:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('Neon:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('Neon:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'BB8SS')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
local Neon_098 = 0
if database:get("Neon:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("Neon:get:mod:"..bot_id..msg.chat_id_)
Neon_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..Neon_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("Neon:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:set('Neon:'..bot_id.."group:link"..msg.chat_id_,text)
...
local wel = database:get('Neon:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
Neon_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..Neon_0[2]..'`}', 1, 'md')
database:set("Neon:not:word:"..bot_id..msg.chat_id_,Neon_0[2])
end

if text == ("الاعدادات") then
if database:get("lock_media:Neon"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('Neon:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('Neon:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'Neon:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'Neon:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:Neon"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_bot:Neon"..msg.chat_id_..bot_id) then
mute_bots = '✔┇'
else
mute_bots = '✖┇'
end
...
if database:get("lock_lllll:Neon"..msg.chat_id_..bot_id) then
mute_flood = '✔┇'
else
mute_flood = '✖┇'
end
------------
if database:get("lock_inline:Neon"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:Neon"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:Neon"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('Neon:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
...
------------
local ex = database:ttl( 'Neon:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local list = database:smembers('Neon:'..bot_id.."repmedia"..msg.chat_id_)  
for k,v in pairs(list) do  
database:del('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('Neon:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)  
database:srem('Neon:'..bot_id.."repmedia"..msg.chat_id_,v)  
end  
...
for k,v in pairs(list) do  
local taha = database:get('Neon:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..taha..'}\n'    
end  
if #list == 0 then  
...
database:set('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = database:get('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
send(msg.chat_id_, msg.id_, 1, "📥┇ ارسل تحذير عند ارسال الكلمه \n", 1, 'md')  
database:set('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"repp")  
database:set('Neon:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
database:sadd('Neon:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = database:get('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test == 'repp' then  
send(msg.chat_id_, msg.id_, 1, '🔖┇تم منع الكلمه لن يتم ارسالها بعد الان  \n', 1, 'md')  
database:del('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = database:get('Neon:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
database:set('Neon:'..bot_id.."filtr1:add:reply:rd"..test..msg.chat_id_, text)  
database:hset('Neon:'..bot_id..'filters:'..msg.chat_id_, text, 'filtered')
end  
database:del('Neon:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
...
database:set('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = database:get('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
...
database:del('Neon:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('Neon:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)  
database:srem('Neon:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
...
database:set('Neon:'..bot_id..'rules'..msg.chat_id_, txt[2])
...
database:set('Neon:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
local hash = 'Neon:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash)
text = "👥┇قائمة المقيدين ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = database:hgetall('Neon'..bot_id.."user:"..v)
...
local hash = 'Neon:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash) 
for k,v in pairs(list) do database:del('Neon:'..bot_id..'res:'..msg.chat_id_) 
...
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(res[2] or 'BB8SS')..')\n☑️┇تم تقييده'
end
else
texts = '✖️┇خطاء'
end
...
database:sadd('Neon:'..bot_id..'res:'..msg.chat_id_, apba[2])
...
local tsX_o = database:get("lock_tag.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التاك بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التاك بالتقييد")
database:set("lock_tag.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الشارحه بالتقيد") then
local tsX_o = database:get("lock_sarha.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الشارحه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الشارحه بالتقييد")
database:set("lock_sarha.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المتحركه بالتقيد") then
local tsX_o = database:get("lock_gif.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل المتحركه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المتحركه بالتقييد")
database:set("lock_gif.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملفات بالتقيد") then
local tsX_o = database:get("lock_files.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملفات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملفات بالتقييد")
database:set("lock_files.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الماركدون بالتقيد") then
local tsX_o = database:get("lock_mark.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الماكدون بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الماركدون بالتقييد")
database:set("lock_mark.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصور بالتقيد") then
local tsX_o = database:get("lock_photo.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصور بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصور بالتقييد")
database:set("lock_photo.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملصقات بالتقيد") then
local tsX_o = database:get("lock_stecker.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملصقات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملصقات بالتقييد")
database:set("lock_stecker.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الفيديو بالتقيد") then
local tsX_o = database:get("lock_video.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الفيديو بالتقييد")
database:set("lock_video.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانلاين بالتقيد") then
local tsX_o = database:get("lock_inline.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانلاين بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانلاين بالتقييد")
database:set("lock_inline.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الدردشه بالتقيد") then
local tsX_o = database:get("lock_chat.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الدردشه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الدردشه بالتقييد")
database:set("lock_chat.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل التوجيه بالتقيد") then
local tsX_o = database:get("lock_fwd.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التوجيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التوجيه بالتقييد")
database:set("lock_fwd.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الاغاني بالتقيد") then
local tsX_o = database:get("lock_audeo.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الاغاني بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الاغاني بالتقييد")
database:set("lock_audeo.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصوت بالتقيد") then
local tsX_o = database:get("lock_voice.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصوت بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصوت بالتقييد")
database:set("lock_voice.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الجهات بالتقيد") then
local tsX_o = database:get("lock_contact.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الجهات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الجهات بالتقييد")
database:set("lock_contact.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل العربيه بالتقيد") then
local tsX_o = database:get("lock_ar.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل العربيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل العربيه بالتقييد")
database:set("lock_ar.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانكليزيه بالتقيد") then
local tsX_o = database:get("lock_en.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانكليزيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانكليزيه بالتقييد")
database:set("lock_en.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الروابط بالتقيد") then
local tsX_o = database:get("lock_link.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الروابط بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الروابط بالتقييد")
database:set("lock_link.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المعرف بالتقيد") then
local tsX_o = database:get("lock_username.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
 tsX000("lock",msg,"☑️┇بالفعل تم قفل المعرف بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المعرف بالتقييد")
database:set("lock_username.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل بصمه الفيديو بالتقيد") then
local tsX_o = database:get("lock_note.note:Neon"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل بصمه الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل بصمه الفيديو بالتقييد")
database:set("lock_note.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_tag.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_sarha.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_gif.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_files.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_mark.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_photo.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_stecker.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_video.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_inline.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_chat.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_fwd.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_audeo.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_voice.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_contact.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_ar.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_en.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_username.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_note.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_link.note:Neon"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
if database:get("lock_audeo.note:Neon"..msg.chat_id_..bot_id) then
 ...
 ------------
if database:get("lock_inline.note:Neon"..msg.chat_id_..bot_id) then
 mute_in = '✔┇'
else
 mute_in = '✖┇'
 end
...
lock_tag = '✖┇'
 ...
 ------------
if database:get("lock_contact.note:Neon"..msg.chat_id_..bot_id) then
lock_contact = '✔┇'
else
lock_contact = '✖┇'
 end
...
database:del('Neon:'..bot_id..'banned:'..msg.chat_id_)
...
if database:sismember('Neon:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif database:sismember('Neon:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif database:sismember('Neon:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif database:sismember('Neon:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
...
if redis:sismember('Neon:'..bot_id..'dev',result.id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('Neon:'..bot_id..'creator:'..msg.chat_id_, result.id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('Neon:'..bot_id..'owners:'..msg.chat_id_, result.id_) then
own = 'مدير ،' else own = '' end
if database:sismember('Neon:'..bot_id..'mods:'..msg.chat_id_, result.id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.id_) then
vip = 'مميز ،' else vip = ''
end
...
database:srem('Neon:'..bot_id..'dev', result.id_)
database:del('Neon:'..bot_id..'sudoo'..result.id_..'', 'no')
database:srem('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_,result.id_)
database:srem('Neon:'..bot_id..'creator:'..msg.chat_id_, result.id_)
database:srem('Neon:'..bot_id..'owners:'..msg.chat_id_, result.id_)
database:srem('Neon:'..bot_id..'mods:'..msg.chat_id_, result.id_)
database:srem('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
if redis:sismember('Neon:'..bot_id..'dev',result.sender_user_id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('Neon:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('Neon:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_) then
own = 'مدير ،' else own = '' end
if database:sismember('Neon:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_) then
vip = 'مميز ،' else vip = ''
end
...
if RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo1' then
database:srem('Neon:'..bot_id..'dev', result.sender_user_id_)
database:del('Neon:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
database:srem('Neon:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
database:srem('Neon:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
database:srem('Neon:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('Neon:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Neon:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo' then
...
------------------------------------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("Neon:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("Neon:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("Neon:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
t = database:get("Neon:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = database:get("Neon:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي '
elseif is_vip(msg) then
t = database:get("Neon:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز '
else
t = database:get("Neon:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك' 
end
...
if not database:get('Neon:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Neon:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((database:get('Neon:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Neon:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((database:get('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = database:get('Neon:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
if msg.sender_user_id_ == tonumber(sudo_add) then
Neon_oop = 'المطور الاساسي'
elseif is_sudo(msg2) then
Neon_oop = database:get("Neon:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
Neon_oop = database:get("Neon:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (database:get("Neon:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("Neon:all_sl:"..database:get("Neon:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
Neon_oop = database:get("Neon:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
Neon_oop = database:get("Neon:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (database:get("Neon:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("Neon:all_sl:"..database:get("Neon:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
Neon_oop = database:get("Neon:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
Neon_oop = database:get("Neon:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg2) then
Neon_oop = database:get("Neon:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg2) then
Neon_oop = database:get("Neon:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
Neon_oop = database:get("Neon:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
if result.id_ then
if not database:get('Neon:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Neon:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((database:get('Neon:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Neon:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((database:get('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = database:get('Neon:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((database:get('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local iameed_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
iameed_text = "غير متفاعل"
...
------------------------------------------------------------------------
if text == ("الرابط") then
if not database:get("Neon:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (database:get('Neon:'..bot_id.."group:link"..msg.chat_id_) and database:get('Neon:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🔘┇ *Group Link* \n['..database:get('Neon:'..bot_id.."group:link"..msg.chat_id_)..']', 1, "md")
...
database:set('Neon:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
local user_msgs = database:get('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('Neon:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if not database:get('Neon:'..bot_id..'id:mute'..msg.chat_id_) then
...
------------------------------------------------------------------------
if text == ("جهاتي") then
add = (tonumber(database:get('Neon:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('Neon:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
...
database:del('Neon:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local Neon = '🗑 ┇ تم حذف سحكاتك '
send(msg.chat_id_, msg.id_, 1, Neon, 1, 'md')
end
------------------------------------------------------------------------
if text == ("اطردني") then
if not database:get('Neon:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('Neon:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('Neon:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
local yess = redis:get('Neon:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('Neon:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Neon:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿┇ لقد تم طردك   ', 1, 'md')
end
end
end
if text and text:match("^لا$") then
local noo = redis:get('Neon:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('Neon:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Neon:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
------------------------------------------------------------------------
if (text and text == (redis:get('Neon:'..bot_id..'name_bot') or 'تشاكي'))then
name_bot = (redis:get('Neon:'..bot_id..'name_bot') or 'تشاكي')
local namebot = {
...
local text_sudo = redis:get('Neon:'..bot_id..'text_sudo')
local nameed = redis:get('Neon:'..bot_id..'nmameed')
local naameed = redis:get('Neon:'..bot_id..'naameed')
if text_sudo then
...
local rules = database:get('Neon:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
if not database:sismember('Neon:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
database:sadd('Neon:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')
local ameed_info = nil
function ameed333(extra,result,success)
ameed_info = '@'..(result.username_ or 'لا يوجد')..''
local function getpro(extra, result, success)
local all_photo_Neon = result.total_count_ 
local tahna = (database:get('Neon:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((database:get('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((database:get('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local iameed_text = "غير متفاعل"
if user_msgs then 
...
t = database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t =  database:get("Neon:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Neon:all_sl:"..database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Neon:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("Neon:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🏻'   
...
if not database:get('Neon:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('Neon:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("Neon:gr:id:text:"..bot_id..msg.chat_id_) then 
local ameed_text = {
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه ",
"وفالله 😔💘",
...
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,"🎥┇ "..ameed_text[ameed3].."\n🎟┇ ايديك • "..msg.sender_user_id_.."\n🎫┇ يوزرك • "..ameed_info.."\n🛰┇ موقعك • "..t.."\n📖┇ رسائلك •("..user_msgs..")\n🗃┇ سحكاتك •("..edit..")\n📓┇ تفاعلك • ("..iameed_text..")\n🤹🏻‍♂️┇ مجوهراتك • ("..nko..")\nꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ",msg.id_,msg.id_.."")
else 
local Neon_new_text = database:get("Neon:gr:id:text:"..bot_id..msg.chat_id_)
local Neon_new_text = Neon_new_text:gsub('#username',(ameed_info or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#edit',(edit or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#stast',(t or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#auto',(iameed_text or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#photos',(all_photo_Neon or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#game',(nko or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,Neon_new_text,msg.id_,msg.id_.."")
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
...
local Neon_new_text = Neon_new_text:gsub('#photos',(all_photo_Neon or 'لا يوجد'))
local Neon_new_text = Neon_new_text:gsub('#game',(nko or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, Neon_new_text, 1, 'html')
end   
end
else
end
...
------------------------------------------------------------------------
if text then
if database:sismember('Neon:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
database:del('Neon:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
...
send(msg.chat_id_, msg.id_, 1, '📜┇معرف ~⪼ ('..(result.username_..')\n🔘┇الاسم ~⪼ ('..result.first_name_ or "Neon")..')', 1, 'md')
end
getUser(memb[2],whois)
end
------------------------------------------------------------------------
if text == ("مشاهده منشور") then
database:set('Neon:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
------------------------------------------------------------------------
if text == 'سمايلات' and database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Neon:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🥖','🥐','🍞','🥨','🍟','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍠','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('Neon:'..bot_id..'klmos'..msg.chat_id_,name)
taha = '🚀┇اسرع واحد يدز » {`'..name..'`}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
------------------------------------------------------------------------
if text == ''..(database:get('Neon:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Neon:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('Neon:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Neon:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'الاسرع' and database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Neon:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Neon:'..bot_id..'klmo'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Neon:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Neon:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('Neon:'..bot_id..'l:id'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل الاسرع ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1) 
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Neon:'..bot_id..'l:id'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'حزوره' and database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Neon:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Neon:'..bot_id..'klmoa'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Neon:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Neon:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('Neon:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Neon:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'المعاني' and database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Neon:'..bot_id..'l:id2'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Neon:'..bot_id..'means'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Neon:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Neon:'..bot_id..'l:id2'..msg.chat_id_) then
if not database:get('Neon:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل المعاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Neon:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'العكس' and database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Neon:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
database:set('Neon:'..bot_id..'aks'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Neon:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Neon:'..bot_id..'l:id3'..msg.chat_id_) then
if not database:get('Neon:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Neon:'..bot_id..'l:id3'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if database:get('Neon:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('Neon:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('Neon:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('Neon:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 5)  
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 5)    
taha = '*🔖┇ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸┇ تم اضافة { 5 } من النقاط *\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:incrby('Neon:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(database:get('Neon:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
database:del('Neon:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('Neon:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if database:get('Neon:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('Neon:'..bot_id.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('Neon:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '*📮┇ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊┇ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 3)  
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 3)    
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:del('Neon:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if text == 'المختلف' and database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','✨','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧜‍♂','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕒','🕤','⌛️','📅',
};
name = katu[math.random(#katu)]
database:del('Neon:'..bot_id..'l:idss'..msg.chat_id_)
database:set('Neon:'..bot_id..':Set_alii:'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == 'امثله' and database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {
'جوز','ضراطه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
database:set('Neon:'..bot_id..':Set_Amthlh:'..msg.chat_id_,name)
database:del('Neon:'..bot_id..'l:idst'..msg.chat_id_)
...
------------------------------------------------------------------------
if text == ''..(database:get('Neon:'..bot_id..':Set_alii:'..msg.chat_id_) or '57999')..'' then -- // المختلف
if not database:get('Neon:'..bot_id..'l:idss'..msg.chat_id_) then 
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('Neon:'..bot_id..':Set_alii:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
database:set('Neon:'..bot_id..'l:idss'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == ''..(database:get('Neon:'..bot_id..':Set_Amthlh:'..msg.chat_id_) or '6499')..'' then -- // امثله
if not database:get('Neon:'..bot_id..'l:idst'..msg.chat_id_) then 
database:incrby('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('Neon:'..bot_id..':Set_Amthlh:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
database:set('Neon:'..bot_id..'l:idst'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == 'محيبس' or text == 'بات' then
if database:get('Neon:'..bot_id..'lock_geam'..msg.chat_id_) then   
Num = math.random(1,6)
database:set('Neon:'..bot_id.."GAMES"..msg.chat_id_,Num) 
TEST = [[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('Neon:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
...
database:set('Neon:'..bot_id.."GAMES:NUM"..msg.chat_id_,Num) 
TEST = '*\n🎮┇ اهلا بك عزيزي في لعبة التخمين :\nٴ━━━━━━━━━━\n'..'⚠┇ ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n'..'🔖┇ سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ \n💥*'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('Neon:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
if text =='مجوهراتي' then 
if tonumber((database:get('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎┇ ليس لديك مجوهرات \n📬┇ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎┇ عدد مجوهراتك الحاليه  ('..(database:get('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎┇ مجموع مجوهراتك  ('..(database:get('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
taha = (database:get('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('Neon:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('Neon:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('Neon:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
...
database:del('Neon:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
database:del('Neon:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
------------------------------------------------------------------------
if text == 'حذف رسائلي' then
database:del('Neon:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
database:del('Neon:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "🗑 ┇ تم حذف رسائلك  ", 1, "md") 
end
------------------------------------------------------------------------
if text == ("الاوامر") and (is_mod(msg) or is_creatorbasic(msg)) then
local help = redis:get('Neon:'..bot_id..'help')
local text =  [[
📮┇هناك {4} اوامر لعرضها
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
local h1 = redis:get('Neon:'..bot_id..'h1')
local text =  [[
...
local h2 = redis:get('Neon:'..bot_id..'h2')
local text =  [[
🥈┇اوامر الادمنيه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚫┇كتم
...
local h3 = redis:get('Neon:'..bot_id..'h3')
local text =  [[
🥇┇ اوامر المدراء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
local hres = redis:get('Neon:'..bot_id..'hres')
local text =[[
...
local h4 = redis:get('Neon:'..bot_id..'h4')
local text =  [[
🎖┇اوامر المطور
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✳️┇تفعيل/تعطيل
...
📜┇[قناه شروحات سورس](http://t.me/BB8SS)

📥┇[قناه ملفات السورس](t.me/BB8SS)
...
database:del( 'Neon:'..bot_id.."charge:"..'-100'..data.channel_.id_)
database:srem("Neon:gog"..bot_id, '-100'..data.channel_.id_)
database:sadd( 'Neon:'..bot_id.."groups",'-100'..data.channel_.id_)
end
end
local chat = {}
if (data.ID == "UpdateNewMessage") then
local Data_Neon = data
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
database:hset('Neon:'..bot_id..'user:'..msg.sender_user_id_,'username',(data.username_ or msg.sender_user_id_))
...
local Bots = database:get("lock_bot:Neon"..msg.chat_id_..bot_id)
for i=0,#mem_id do  
...
if msg.content_.ID == "MessageChatAddMembers" then  
local mem_id = msg.content_.members_  
local Bots = database:get("lock_botAndBan:Neon"..msg.chat_id_..bot_id) 
for i=0,#mem_id do  
if msg.content_.members_[i].type_.ID == "UserTypeBot" and not is_mod(msg) and Bots then   
...
if database:get("Neon:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local Neon_edit_text = database:get("Neon:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
print(Neon_edit_text)
if Neon_edit_text then
data.message_.content_.text_ = (Neon_edit_text or data.message_.content_.text_)
end
end
end
local Name_Bot = (redis:get('Neon:'..bot_id..'name_bot') or 'تشاكي')
...
local msgg = result 
database:incr('Neon:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
if database:get("lock_edit:Neon"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/BB8SS)" , 1, 'md') 
end
end
getUser(result.sender_user_id_,get_edit)
delete_msg(msg.chat_id_,{[0] = msg.message_id_}) 
elseif database:get("lock_edit:Neon:kick"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
function get_edit(arg,data)
local username = data.username_
local name = data.first_name_
local iduser = data.id_
...
------------------------------------------------------------------------
local taha = database:get('Neon:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
...
------------------------------------------------------------------------
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then  local list = database:smembers('Neon:'..bot_id.."userss") for k,v in pairs(list) do tdcli_function({ID='GetChat',chat_id_ = v},function(arg,data) end,nil) end 
local list = database:smembers('Neon:'..bot_id..'groups') 
for k,v in pairs(list) do 
...
database:srem("Neon:gog"..bot_id,v) 
database:srem('Neon:'..bot_id..'pro:groups',v) 
database:srem( 'Neon:'..bot_id.."groups",v) 
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
database:srem("Neon:gog"..bot_id,v) 
database:srem('Neon:'..bot_id..'pro:groups',v) 
...
database:sadd("Neon:gog"..bot_id,v) 
database:sadd( 'Neon:'..bot_id.."groups",v) 
...
local msg = data.message_
local text = msg.content_.text_
local Get_Msg_Pin = database:get('Neon:'..bot_id..'Msg_Text'..msg.chat_id_)
if Get_Msg_Pin ~= nil then
if text == Get_Msg_Pin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) if d.ID == 'Ok' then;database:del('Neon:'..bot_id..'Msg_Text'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
print(msg.content_.sticker_.sticker_.persistent_id_)
if Get_Msg_Pin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) database:del('Neon:'..bot_id..'Msg_Text'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.voice_) then
